...
from app.models import market as models_market
...
def test_get_stock_quotes_success(mock_get_quotes):
    quote = models_market_data.StockQuote.model_construct(currency="AUD", market_price=110.5)
...
def test_get_symbol_overview_success(mock_get_overview):
    overview = models_stocks.SymbolOverview.model_construct(short_name="CBA", long_name="Commonwealth Bank")
...
def test_get_symbol_info_success(mock_get_info):
    info = models_stocks.SymbolInfo.model_construct(short_name="CBA", long_name="Commonwealth Bank")
...
def test_get_quote_at_date_success(mock_get_quote):
    history_point = models_market_data.HistoryPoint.model_construct(
        timestamp=1705276800,
        timestamp_str="2025-01-15",
        close=105.0,
    )
...
def test_get_symbol_history_success(mock_get_history):
    points = [
        models_market_data.HistoryPoint.model_construct(
            timestamp=1705276800,
            timestamp_str="2025-01-15",
            close=105.0,
        ),
...
@patch("app.routers.stocks.market_service.market_indices")
def test_get_index_companies_success(mock_market_indices):
    company = models_market.CompanyBriefInfo(
        symbol="CBA.AX",
